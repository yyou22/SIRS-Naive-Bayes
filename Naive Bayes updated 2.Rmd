...
cor(c)
```
From the correlation matrix, we can see that most factors are not strongly correlated to oters. Also, naive bayes algorithm is a robust approach, so our model using temperature, heartrate, resprate, paco2 and wbc is still working.
